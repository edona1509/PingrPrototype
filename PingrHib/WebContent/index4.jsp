	  	 ...
	  	 google.maps.event.addListener(marker, 'click',  (function(marker, showContent, up, down, idGiusto) {
	  		  return function() {
	                infowindow.setContent('<p>Ping: '+ showContent +'</p>'+ 
	        				...
	        				'<input type="submit" value="Up vote" onclick="voteUp('+ up + ','+ idGiusto +','+ down+')" form="myFormUp" ></input>'+
	          				'<input type="submit" value="Down vote" id="voteDownButton" onclick="voteDown('+ down + ','+idGiusto+ ','+ up+')"  form="myFormDown"></input>');
